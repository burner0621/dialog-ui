...
export class Service extends RestService {
  ...
  getXlsDetail(info) {
    ...
    if (info.supplierCode !== null && info.divisionId !== null) {
      query = `?supplierCode=${info.supplierCode}&divisionId=${info.divisionId}&month=${info.month}&year=${info.year}`;
    } else if (info.supplierCode !== null && info.divisionId === null) {
      query = `?supplierCode=${info.supplierCode}&month=${info.month}&year=${info.year}`;
    } else if (info.supplierCode === null && info.divisionId !== null) {
      query = `?divisionId=${info.divisionId}&month=${info.month}&year=${info.year}`;
    } else {
      query = `?month=${info.month}&year=${info.year}`;
    }

    let endpoint = `${serviceUri}/detail/xls${query}&isImport=${true}`;
    return super.getXls(endpoint);
  }
