...
export class Create {
  ...
  saveCallback(event) {
    ...
    if (fileList[0] == undefined) {
      ...
      var endpoint = "master/accounting-categories/upload";
      var request = {
        method: "POST",
        headers: {},
        body: formData,
      ...
      this.service.endpoint.client.fetch(endpoint, request).then((response) => {
        if (response.status == 200) {
          var getRequest = this.service.endpoint.client.fetch(
            endpoint,
            request
          );
