...
!function ($) {
  ...
  BootstrapTable.prototype.showContextMenu = function (params) {
    ...
    var selector = params.contextMenu || this.options.contextMenu;

    var newMenu = that.$tableContainer.parent().siblings(`${selector}`);
    if (newMenu.length > 0) {
      var auId = newMenu.attr("au-target-id");
      selector = `${selector}[au-target-id=${auId}]`;
      $('body').children(selector).remove();
    }
    ...
    // console.log($menu.attr("au-target-id"));
    // console.log($(this.options.contextMenu));
    // Show the menu
    $menu.data('index', $tr.data('index'))
      // .appendTo(that.$tableContainer)
      .appendTo($('body'))
      .css({
        position: "absolute",
